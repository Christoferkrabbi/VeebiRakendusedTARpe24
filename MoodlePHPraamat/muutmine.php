...
$kask = $yhendus->prepare("INSERT INTO lehed (pealkiri, sisu, kuupäev, autor) VALUES (?, ?, ?, ?)");
$kask->bind_param("ssss", $_REQUEST["pealkiri"], $_REQUEST["sisu"], $_REQUEST["kuupäev"], $_REQUEST["autor"]);
$kask->execute();
...
$kask = $yhendus->prepare("UPDATE lehed SET pealkiri=?, sisu=?, kuupäev=?, autor=? WHERE id=?");
$kask->bind_param(
"ssiss",
$_REQUEST["pealkiri"],
$_REQUEST["sisu"],
    ...
    if (isset($_REQUEST["id"])) {
        ...
        if ($kask->fetch()) {
            if (isset($_REQUEST["muutmine"])) {
                ...
                echo "<br />";
                echo htmlspecialchars($kuupäev);
                echo "<br />";
                echo htmlspecialchars($autor);
                echo "<br />";
                echo "<img src='$pilt' alt='pilt' width='200' height='200' />";
